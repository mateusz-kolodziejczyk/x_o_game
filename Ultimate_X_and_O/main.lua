...
local function drawLine(x1, y1, x2, y2, width, color, centerPos)
    ...
    local line = display.newLine(backGroup,
        centerPos.x + x1*size, centerPos.y + y1*size,
        centerPos.x + x2*size, centerPos.y + y2*size
    )
...
drawBoard = function(scale, color, centerPos)
        -- Vertical lines
        drawLine(scale*-1/2, scale*-3/2, scale*-1/2, scale*3/2, scale*8, color, centerPos)
        drawLine(scale*1/2, scale*-3/2, scale*1/2, scale*3/2, scale*8, color, centerPos)
        -- Horizontal lines
        drawLine(scale*-3/2, scale*-1/2, scale*3/2, scale*-1/2, scale*8, color, centerPos)
        drawLine(scale*-3/2, scale*1/2,scale*3/2, scale*1/2, scale*8, color, centerPos)
end
...
-- Populates boards with clickable squares
local function populateBoard(scale, centerPos, color, alpha)
    drawBoard(scale, color, centerPos)
    for k = 1, 9 do
        local row, col = mylib.k2rc(k)
        local x = centerPos.x + (col-4/2)*size*subboardScale
        local y = centerPos.y + (row-4/2)*size*subboardScale
        local rect = display.newRect(uiGroup, x, y, (size-gap)*scale, (size-gap)*scale)
        rect.alpha = alpha or 0.05
        rect.k = k
...
local function createBoard()
    ...
    populateBoard(1, {x = display.contentCenterX, y = display.contentCenterY})
