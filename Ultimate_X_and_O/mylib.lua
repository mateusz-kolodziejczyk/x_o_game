...
function mylib.isRowWin(board)
	for r = 0,2 do
		if board[r*3+1]~=0 and board[r*3+1]~=math.huge and board[r*3+1]==board[r*3+2] and board[r*3+2]==board[r*3+3] then
            return r+1
		end
	end
	return 0
...
function mylib.isColWin(board)
	for c = 1, 3 do
		if board[c]~=0 and board[c]~=math.huge and board[c]==board[3+c] and board[3+c]==board[6+c] then
            return c
		end
	end
	return 0
...
function mylib.isDiagonalWin(board)
	return board[1]~=0 and board[1]~=math.huge and board[1]==board[5] and board[5]==board[9]
end


function mylib.isAntiDiagonalWin(board)
	return board[3]~=0 and board[3]~=math.huge and board[3]==board[5] and board[5]==board[7]
end
